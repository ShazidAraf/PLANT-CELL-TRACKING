...
def extract_cell(data,seg,l):
    ...
    xl = np.clip(box[0]-dw,0,511)
    xh = np.clip(box[1]+dw,0,511)
    yl = np.clip(box[2]-dh,0,511)
    yh = np.clip(box[3]+dh,0,511)
...
def get_neighbour(seg_slice, cell_label,background,bx):
    ...
    b = (np.clip(boundary[:,0],0,size-1),np.clip(boundary[:,1],0,size-1))
