# python train_graph_pair.py --epoch 200 --lr 1e-3 --batch_size 50
...
def parse_args():
    ...
    parser.add_argument('--ckp', type=str, default='checkpoint_graph_pair')
...
def loss_val_point(out,label):
    ...
    plt.savefig("train_pair_point.png")
...
def loss_val_graph(out,label):
    ...
    plt.savefig("train_pair_graph.png")


    # print('graph mean: ', np.mean(match),  np.mean(not_match))
    # print('graph std: ', np.std(match),  np.std(not_match))
...
def train(epoch, train_loader, model, optimizer, scheduler=None):
    ...
    for i, data_batch in enumerate(train_loader):
        ...
        # pt0, knn_pt0, pt1, knn_pt1,mask, label, label_point = to_cuda(data_batch,0)
        label_point = label_point.view(-1)
        label = label.view(-1)
        ...
        loss1 = criterion(out1,label_point)
...
def evaluate(loader, model,th):
    ...
    with torch.no_grad():
        ...
        for i, data_batch in enumerate(loader):
            ...
            point_match.append(calc_match(out1,label_point,th))
            graph_match.append(calc_match(out2,label,th))

            all_points.append(len(label_point))
            all_graphs.append(len(label))

    accuracy_graph = np.sum(graph_match)*100/np.sum(all_graphs)

    accuracy_point = np.sum(point_match)*100/np.sum(all_points)

    print('Val point accuracy: ',accuracy_point)
...
if __name__ == '__main__':
    ...
    model = Graph_Pair_Model(input_dim=3,n_points=args.n_points,n_edges = 16, output_dim = 512)
    ...
    for i in range(args.epoch):
        ...
        # train_idx = [0,1,2,3]
        # val_idx = [4]
        train_data = Graph_Pair_loader(MAIN_DIR,split = 'train',plant_idx=train_idx,n_points=args.n_points-1,k=args.k,max_dist=args.max_dist)
        val_data = Graph_Pair_loader(MAIN_DIR,split = 'val',plant_idx=val_idx,n_points=args.n_points-1,k=args.k,max_dist=args.max_dist)
